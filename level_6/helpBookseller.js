...
// TESTS
stockList(listOfArt, listOfCat); // "(A : 0) - (B : 1290) - (C : 515) - (D : 600)"
...
function stockList(listOfArt, listOfCat){
    ...
    listOfArt.forEach(el => {
        let numbers = Number(el.split(" ")[1]);
        if(listOfCat.includes(el[0])){
            obj[el[0]] === undefined ? obj[el[0]] = numbers : obj[el[0]] = obj[el[0]] + numbers;
        }
    });

    for(let i = 0; i < listOfCat.length; i++){
        result= result + "(" + listOfCat[i] + " : " + (obj[listOfCat[i]] || 0 )+ ")  ";
    }
